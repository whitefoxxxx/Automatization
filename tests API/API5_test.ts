...
Feature('Папка в папке');

Scenario.only('Создать папку+папку+файл, удалить', async () => {
  await api.createFolder('test');
  ...
  await api.createFolder('test', 'foo');
  const createdFolder2 = await api.getFolder({
    folderName: 'test',
    folderName2: 'foo',
    isImage: false,
  });
  await data.isExists({
    response: createdFolder2,
    isExists: true,
    folderName: 'test',
    folderName2: 'foo',
  });
  //Создала папку в папке и проверила, создались ли
